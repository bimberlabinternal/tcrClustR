...
RunTcrClustering <- function(seuratObj_TCR = NULL,
  ...
  if (is.null(chainsToCluster)) {
    ...
    assaysToCluster <- paste0(chainsToCluster, c("_fl", "_cdr3"))
  ...
  for (assayName in assays) {
    ...
    # detect chains present in assay name
    all_possible_chains <- c("TRA", "TRB", "TRG", "TRD")
    chains_in_assay <- sort(head(unlist(strsplit(assayName, split = '-|\\.')),-1))
    if (any(! chains_in_assay %in% all_possible_chains)) {
      stop(paste0('Unknown chains in assay name: ', assayName))
    }

    # skip if no chains detected
    if (length(chains_in_assay) == 0) {
      stop("Could not detect chain type from assay ", assayName)
    }

    # Now store back into the seurat object, top-level and assay-level:
    chainId <- paste0(chains_in_assay, collapse = '-')
    cloneIdxCol <- paste0(chainId, '-CloneIdx')
    clusterIdxCol <- paste0(chainId, '-ClusterIdx')
    ...
    toAddMeta <- data.frame(CloneIdx = seuratObj_TCR@meta.data[[cloneIdxCol]], row.names = rownames(seuratObj_TCR@meta.data)) |>
      dplyr::left_join(toAdd, by = cloneIdxCol)

    if (any(rownames(toAddMeta) != rownames(seuratObj_TCR@meta.data))) {
      stop('Expected rownames to match after left_join')
    }

    seuratObj_TCR <- Seurat::AddMetaData(seuratObj_TCR, toAddMeta)
    rm(toAddMeta)

    # Then the assay object:
    ...
    toAddAssay <- data.frame(CloneIdx = rownames(ad), row.names = rownames(ad)) |>
      dplyr::left_join(toAdd, by = cloneIdxCol)
    toAddAssay[[cloneIdxCol]] <- NULL
