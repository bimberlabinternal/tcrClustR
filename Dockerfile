...
RUN apt-get update && apt-get install -y r-base r-base-dev && \
    ...
    Rscript -e "install.packages(c('clusterCrit', 'dbscan', 'cluster', 'arrow', 'RColorBrewer', 'patchwork', 'dirichletprocess', 'egg', 'ggraph', 'tidygraph', 'lme4', 'marginaleffects'), lib='/usr/local/lib/R/site-library', dependencies=TRUE, ask = FALSE)" && \
